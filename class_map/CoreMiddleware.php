...
class CoreMiddleware extends HttpCoreMiddleware
{
     ...
     */
    protected function handleResponse(?Message $message, $httpStatus = 200, int $grpcStatus = 0, string $grpcMessage = ''): ResponseInterface
    {
        return $this->response()->withStatus($httpStatus)
            ->withBody(new SwooleStream(\DtmClient\Grpc\GrpcParser::serializeMessage($message)))
